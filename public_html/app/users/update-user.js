$(document).ready(function(){
   ...
   $(document).on('click','.update-user-button',function(){
       ...
       $.getJSON("http://localhost:8080/NetBeans/Moodle/RestApiPhpTestApp/api/user/read_one.php?userID=" + id, function(data){
           ...
           $.getJSON("http://localhost:8080/NetBeans/Moodle/RestApiPhpTestApp/api/user/readUser.php", function(data){
              var companies_option_html = `<select name='Company' class= 'form-control'>`;
              $.each(data.records, function(key,val){
                 //preselect option if company id is the same
                 if(val["Company_Name"]===company){companies_option_html+=`<option name = 'Company' value='` + val["Company_Name"]  + `'selected>`+val["Company_Name"] +`</option>`;}
                 else{companies_option_html+=`<option name = 'Company' value='`+val["Company_Name"]  +`'>`+val["Company_Name"]  + `</option>`;}
              });
